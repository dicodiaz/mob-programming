...
const atomicNumber = (num) => {
  const result = [];
  let electronsPlaced = 0, n = 1;
  while (electronsPlaced < num) {
    const numberOfElectronsInStep = 2 * n * n > (num - electronsPlaced) ? num - electronsPlaced : 2 * n * n;
    result.push(numberOfElectronsInStep);
    electronsPlaced += numberOfElectronsInStep;
    n++;
  }
  return result;
}
